...
- name: Create envfile
  ...
  with:
    envkey_DEBUG: false
    envkey_EXPO_PUBLIC_API_URL: ${{ secrets.PRODUCTION_API_URL }}
    file_name: .env
...
This action will create a new `.env` file with the variables you listed (for example `DEBUG` and `EXPO_PUBLIC_API_URL`). Include every variable your `env.ts` / build expects.

## Create new release

For QA release, Go to your GitHub repo actions tab and run `new-app-version` workflow with the your desired release type.

![new-app-version](https://github.com/avy-erp/react-native-template-avy-erp/assets/11137944/efc62eda-a465-44ab-a185-860cfd2e9099)
...
![new-github-release](https://github.com/avy-erp/react-native-template-avy-erp/assets/11137944/d3b9d8ff-37f8-4551-97c1-4c9fbc8860d3)
