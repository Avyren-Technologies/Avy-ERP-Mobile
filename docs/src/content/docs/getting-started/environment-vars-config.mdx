...
- **Non-prefixed variables** (build-time secrets): ONLY accessible in `app.config.ts` at build-time, NOT in your client code
...
All environment variables are strings by default. Convert them to the correct type in `_env`, for example:
```ts
// For numbers
EXPO_PUBLIC_MY_NUMBER: Number(process.env.EXPO_PUBLIC_MY_NUMBER ?? 0),

// For booleans
EXPO_PUBLIC_MY_FLAG: process.env.EXPO_PUBLIC_MY_FLAG === 'true',
```
:::
...
const BUNDLE_IDS = {
  development: 'com.avy_erp.development',
  preview: 'com.avy_erp.preview',
  production: 'com.avy_erp',
} as const;
...
See the [migration guide](https://github.com/avy-erp/react-native-template-avy-erp) for detailed instructions.
