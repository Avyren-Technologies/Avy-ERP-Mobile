...
const logger = createLogger('MfaSetupScreen');
const CODE_LENGTH = 6;
...
export function MfaSetupScreen() {
    ...
    const fetchSetup = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          showErrorMessage('Failed to load MFA setup. Please try again.');
          router.replace('/login');
        }
      } catch (err: any) {
        const message =
        ...
        logger.error('MFA setup fetch failed', { message });
  ...
  const handleSubmit = async (code: string) => {
    ...
    setSubmitting(true);
    try {
      const response = await authApi.confirmMfa(code, mfaToken);
      ...
      if (response.success && response.data) {
        ...
        if (user && tokens) {
          ...
          logger.info('MFA setup confirmed — navigating to app');
          router.replace('/(app)');
          return;
        }
      }
      ...
      logger.error('MFA confirm failed', { message });
      showErrorMessage(message);
      setDigits(Array(CODE_LENGTH).fill(''));
      inputRefs.current[0]?.focus();
    } finally {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text className="font-inter mt-4 text-sm" style={styles.loadingText}>
                Loading MFA setup...
              </Text>
            </View>
          ) : (
