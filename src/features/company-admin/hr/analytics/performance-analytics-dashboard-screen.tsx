...
export function PerformanceAnalyticsDashboardScreen() {
  ...
  const { data: response, isLoading, refetch } = useAnalyticsDashboard('performance', filters);
  ...
  if (!isLoading && !dashboardData?.kpis?.length && !dashboardData?.meta?.lastComputedAt) {
    return (
      <DashboardShell title="Performance Analytics" loading={false}>
        <ZeroDataState
          title="No performance data yet"
          message="Performance analytics will appear once appraisal cycles and goals are configured."
          icon="chart"
          action={{
            label: 'Go to Appraisal Cycles',
            onPress: () => router.push('/company/hr/appraisal-cycles'),
          }}
        />
      </DashboardShell>
    );
  ...
  return (
    <DashboardShell title="Performance Analytics" loading={isLoading} onRefresh={handleRefresh}>
      ...
      {/* Rating Trend + Goal Completion Trend */}
