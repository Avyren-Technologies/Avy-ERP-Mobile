        ...
        <Modal visible={visible} transparent={false} animationType="slide" onRequestClose={onClose} presentationStyle="fullScreen">
            <View style={{ flex: 1, backgroundColor: colors.white }}>
                <View style={[styles.modalHeader, { paddingTop: insets.top + 8 }]}>
                    ...
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white uppercase tracking-tighter">Apply Leave</Text>
                    <View style={{ width: 40 }} />
                </View>
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 60 }}>
                        ...
                        <View style={styles.toggleRow}>
                            <View style={{ flex: 1 }}>
                                <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white uppercase">Half Day Application</Text>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-tighter">REASON FOR LEAVE <Text className="text-danger-500">*</Text></Text>
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20 }]}>
                    ...
                    <View style={styles.fieldWrap}>
                        <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Note <Text className="text-danger-500">*</Text></Text>
            ...
            <View style={styles.card}>
                <View style={styles.cardHeader}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
                        ...
                        <View style={{ flex: 1 }}>
                            ...
                            <Text className="mt-0.5 font-inter text-xs text-neutral-500 dark:text-neutral-400">{item.leaveTypeName}</Text>
                        </View>
                    </View>
                ...
                <View style={styles.dateRow}>
                    <View style={styles.dateChip}>
                        ...
                        <Text className="font-inter text-xs text-neutral-600 dark:text-neutral-400">{item.fromDate.split('T')[0]} to {item.toDate.split('T')[0]}</Text>
                    </View>
    ...
    const requests: LeaveRequestItem[] = React.useMemo(() => {
        ...
        return raw.map((item: any) => {
            ...
            const empName = item.employeeName
                ...
                ?? '';
            // Extract leave type name from nested object
            const ltName = item.leaveTypeName
                ?? item.leaveType?.name
                ?? leaveTypeOptions.find((o: any) => o.id === item.leaveTypeId)?.label
                ?? '';
            // Normalize status: PENDING → Pending
            ...
            return {
                ...
                employeePhoto: item.employeePhoto ?? item.employee?.profilePhotoUrl ?? '',
                ...
                halfDay: item.isHalfDay ?? item.halfDay ?? false,
...
const createStyles = (isDark: boolean) => StyleSheet.create({
    ...
    backBtn: { width: 40, height: 40, borderRadius: 12, backgroundColor: isDark ? colors.primary[900] : colors.primary[50], justifyContent: 'center', alignItems: 'center' },
    ...
    daysBadge: { backgroundColor: isDark ? colors.primary[900] : colors.primary[50], borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
    ...
    calcDaysBadge: { backgroundColor: isDark ? colors.primary[900] : colors.primary[50], borderRadius: 12, padding: 10, alignItems: 'center', marginBottom: 14 },
