...
const PATTERN_COLORS: Record<RosterPattern, { bg: string; text: string }> = {
    ...
    CUSTOM: { bg: colors.neutral[100], text: 'text-neutral-600 dark:text-neutral-400' },
};
            ...
            <View style={{ flex: 1, backgroundColor: colors.white }}>
                ...
                <View style={[styles.modalHeader, { paddingTop: insets.top + 10 }]}>
                    ...
                    <View style={{ flex: 1, marginLeft: 16 }}>
                        ...
                        <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400"> Configure work schedules and off patterns </Text>
                    </View>
                </View>
                ...
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 24, paddingBottom: insets.bottom + 100 }}>
                        <View style={styles.fieldWrap}>
                            <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-wider">Roster Name <Text className="text-danger-500">*</Text></Text>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-wider">Pattern</Text>
                            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                                {PATTERNS.map(opt => {
                                    const selected = opt === pattern;
                                    return (
                                        <Pressable key={opt} onPress={() => setPattern(opt)} style={[styles.chip, selected && styles.chipActive]}>
                                            <Text className={`font-inter text-xs font-semibold ${selected ? 'text-white' : 'text-neutral-600 dark:text-neutral-400'}`}>{PATTERN_LABELS[opt].toUpperCase()}</Text>
                                        </Pressable>
                                    );
                                })}
                            </View>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-wider">Applicable Employee Types</Text>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-wider">Effective From</Text>
                            <View style={styles.inputWrap}>
                        ...
                        <View style={styles.toggleRow}>
                            <View style={{ flex: 1, marginRight: 12 }}>
                                <Text className="font-inter text-sm font-semibold text-primary-950 dark:text-white uppercase tracking-tight">Set as Default Roster</Text>
                                <Text className="mt-0.5 font-inter text-[11px] text-neutral-500 dark:text-neutral-400">Auto-assigned to new employees during onboarding</Text>
                            </View>
            ...
            <Pressable onPress={onEdit} style={({ pressed }) => [styles.card, pressed && styles.cardPressed]}>
                ...
                <View style={styles.cardMeta}>
                    <View style={styles.metaChip}>
                        <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400 font-medium">OFF: {DAY_LABELS[item.weekOff1?.toUpperCase()] || item.weekOff1}{item.weekOff2 && item.weekOff2 !== 'NONE' ? `, ${DAY_LABELS[item.weekOff2.toUpperCase()] || item.weekOff2}` : ''}</Text>
                    </View>
                    {item.applicableTypes && item.applicableTypes.length > 0 && (
                        <View style={styles.metaChip}>
                            <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400 font-medium">TYPES: {item.applicableTypes.join(', ').toUpperCase()}</Text>
                        </View>
                    )}
                </View>
            </Pressable>
