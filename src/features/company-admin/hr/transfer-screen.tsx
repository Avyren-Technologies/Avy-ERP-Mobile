        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20, maxHeight: '85%' }]}>
                    <View style={styles.sheetHandle} />
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-4">Initiate Transfer</Text>
            ...
            <View style={styles.card}>
                ...
                <View style={styles.flowRow}>
                    <View style={{ flex: 1 }}>
                        ...
                        {item.fromDepartment ? <Text className="font-inter text-xs font-semibold text-neutral-600 dark:text-neutral-400">{item.fromDepartment}</Text> : null}
                        {item.fromDesignation ? <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400">{item.fromDesignation}</Text> : null}
  ...
  const styles = createStyles(isDark);
    ...
    const renderHeader = () => (
        <Animated.View entering={FadeInDown.duration(400)} style={styles.headerContent}>
            <Text className="font-inter text-2xl font-bold text-primary-950 dark:text-white">Employee Transfers</Text>
            <Text className="mt-1 font-inter text-sm text-neutral-500 dark:text-neutral-400">{items.length} transfer{items.length !== 1 ? 's' : ''}</Text>
