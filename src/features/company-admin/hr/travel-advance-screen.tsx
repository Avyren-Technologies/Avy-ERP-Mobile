...
function SettledBadge({ settled }: { settled: boolean }) {
    return (
        <View style={[styles.typeBadge, { backgroundColor: settled ? colors.success[50] : colors.neutral[100] }]}>
            <Text className={`font-inter text-[10px] font-bold ${settled ? 'text-success-700' : 'text-neutral-500 dark:text-neutral-400'}`}>{settled ? 'Settled' : 'Unsettled'}</Text>
        </View>
    );
}
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20, maxHeight: '80%' }]}>
                    ...
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-3">Select Employee</Text>
                        ...
                        renderItem={({ item }) => (
                            ...
                            >
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400">{item.employeeId ?? item.id}</Text>
                                </View>
                            </Pressable>
                        )}
                        ListEmptyComponent={<Text className="font-inter text-sm text-neutral-500 dark:text-neutral-400 text-center py-4">No employees found</Text>}
                    />
                </View>
            </View>
        </Modal>
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20, maxHeight: '92%' }]}>
                    <View style={styles.sheetHandle} />
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-4">New Travel Advance</Text>
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                        ...
                        <View style={styles.fieldWrap}>
                            ...
                            <Pressable onPress={() => setEmpPickerVisible(true)} style={styles.inputWrap}>
                                <Text className={`font-inter text-sm ${employeeId ? 'text-primary-950 dark:text-white' : 'text-neutral-400'}`}>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Trip Purpose <Text className="text-danger-500">*</Text></Text>
                        ...
                        <View style={styles.fieldWrap}>
                            <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Trip Destination</Text>
                        ...
                        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 14 }}>
                            <View style={{ flex: 1 }}>
                                <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Travel From</Text>
                            ...
                            <View style={{ flex: 1 }}>
                                <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Travel To</Text>
                        ...
                        {showBreakdown && (
                            <View style={styles.breakdownCard}>
                                <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                                    <View style={{ flex: 1 }}>
                                        <Text className="mb-1 font-inter text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase">Accommodation</Text>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text className="mb-1 font-inter text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase">Transport</Text>
                                ...
                                <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                                    <View style={{ flex: 1 }}>
                                        <Text className="mb-1 font-inter text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase">Meals</Text>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text className="mb-1 font-inter text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase">Other</Text>
                                ...
                                <View style={{ borderTopWidth: 1, borderTopColor: colors.neutral[200], paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text className="font-inter text-[10px] font-bold text-neutral-500 dark:text-neutral-400 uppercase">Estimated Total</Text>
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20 }]}>
                    <View style={styles.sheetHandle} />
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-4">Settle Advance</Text>
                    <View style={{ backgroundColor: colors.neutral[50], borderRadius: 14, padding: 16, marginBottom: 16 }}>
                        <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400 mb-1">Employee</Text>
                        <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white">{advance.employeeName ?? advance.employeeId}</Text>
                        <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400 mt-3 mb-1">Advance Amount</Text>
                        <Text className="font-inter text-lg font-bold text-primary-600">{'₹'}{Number(advance.amount ?? 0).toLocaleString('en-IN')}</Text>
                        {advance.tripPurpose ? (
                            <>
                                <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400 mt-3 mb-1">Trip Purpose</Text>
                                <Text className="font-inter text-sm text-primary-950 dark:text-white">{advance.tripPurpose}</Text>
                            </>
                        ) : null}
                    </View>
                    <View style={styles.fieldWrap}>
                        <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Expense Claim ID (optional)</Text>
        ...
        <Modal visible={visible} transparent animationType="fade">
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                <View style={{ backgroundColor: colors.white, borderRadius: 24, padding: 28, width: '85%', alignItems: 'center' }}>
                    ...
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-4">Settlement Summary</Text>
                    <View style={{ backgroundColor: colors.neutral[50], borderRadius: 14, padding: 16, width: '100%', marginBottom: 16 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                            <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400">Original Advance</Text>
                            <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white">{'₹'}{Number(result.advanceAmount).toLocaleString('en-IN')}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                            <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400">Expense Claim</Text>
                            <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white">{'₹'}{Number(result.claimAmount).toLocaleString('en-IN')}</Text>
                        </View>
                        <View style={{ borderTopWidth: 1, borderTopColor: colors.neutral[200], paddingTop: 8, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400">Difference</Text>
                            <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white">{'₹'}{Math.abs(result.difference).toLocaleString('en-IN')}</Text>
                        </View>
                    </View>
        ...
        <Animated.View entering={FadeInUp.duration(350).delay(100 + index * 60)}>
            <View style={styles.card}>
                <View style={styles.cardHeader}>
                    <View style={{ flex: 1 }}>
                        <Text className="font-inter text-sm font-bold text-primary-950 dark:text-white" numberOfLines={1}>{item.employeeName ?? item.employeeId}</Text>
                        <Text className="mt-1 font-inter text-xs text-neutral-500 dark:text-neutral-400" numberOfLines={1}>{item.tripPurpose || 'No purpose specified'}</Text>
                    </View>
  ...
  const styles = createStyles(isDark);
    ...
    const renderHeader = () => (
        <Animated.View entering={FadeInDown.duration(400)} style={styles.headerContent}>
            <View>
                <Text className="font-inter text-2xl font-bold text-primary-950 dark:text-white">Travel Advances</Text>
                <Text className="mt-1 font-inter text-sm text-neutral-500 dark:text-neutral-400">{advances.length} advance{advances.length !== 1 ? 's' : ''}</Text>
            </View>
...
const createStyles = (isDark: boolean) => StyleSheet.create({
    ...
    breakdownToggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: isDark ? '#1E1B4B' : colors.neutral[50], borderRadius: 12, borderWidth: 1, borderColor: isDark ? colors.neutral[700] : colors.neutral[200], paddingHorizontal: 14, paddingVertical: 12, marginBottom: 14 },
    breakdownCard: { backgroundColor: isDark ? '#1E1B4B' : colors.neutral[50], borderRadius: 14, padding: 14, marginBottom: 14, borderWidth: 1, borderColor: isDark ? colors.neutral[800] : colors.neutral[100] },
});
