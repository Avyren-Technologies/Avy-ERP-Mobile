      ...
      >
        <View style={cardStyles.cardHeader}>
          ...
          >
            ...
            <Text className="ml-1 font-inter text-[10px] font-bold text-white">Check Out</Text>
          </Pressable>
        </View>
        ...
        <View style={cardStyles.cardMeta}>
          {item.visitCode ? (
            <View style={cardStyles.metaChip}>
              <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400">Code: {item.visitCode}</Text>
            </View>
          ) : null}
          {item.badgeNumber ? (
            <View style={cardStyles.metaChip}>
              <Text className="font-inter text-[10px] text-neutral-500 dark:text-neutral-400">Badge: {item.badgeNumber}</Text>
            </View>
          ) : null}
          {item.checkInTime ? (
            <View style={cardStyles.metaChip}>
...
export function QuickCheckOutScreen() {
  ...
  const renderHeader = () => (
    <Animated.View entering={FadeInDown.duration(400)} style={s.headerContent}>
      <Text className="font-inter text-2xl font-bold text-primary-950 dark:text-white">Quick Check-Out</Text>
      ...
      <View style={{ marginTop: 16 }}>
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search by name, company, code, badge..." />
      </View>
    </Animated.View>
  );
  ...
  return (
    <View style={s.container}>
      ...
      <AppTopHeader title="Quick Check-Out" onMenuPress={toggle} />
...
const cardStyles = StyleSheet.create({
  ...
  cardPressed: { backgroundColor: colors.warning[50], transform: [{ scale: 0.98 }] },
