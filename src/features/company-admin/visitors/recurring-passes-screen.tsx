...
import { useCreateRecurringPass, useRevokeRecurringPass } from '@/features/company-admin/api/use-visitor-mutations';
import { useRecurringPasses } from '@/features/company-admin/api/use-visitor-queries';
...
function CreatePassModal({
  ...
  React.useEffect(() => {
    if (visible) {
      setVisitorName(''); setVisitorCompany(''); setPassType('MONTHLY'); setHostName(''); setErrors({});
    }
  }, [visible]);
  ...
  const validate = () => {
    ...
    if (!hostName.trim()) e.hostName = 'Host employee is required';
    setErrors(e);
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 24, paddingBottom: insets.bottom + 100 }}>
            ...
            <View style={formStyles.fieldWrap}>
              ...
              <View style={formStyles.inputWrap}>
                <TextInput style={[formStyles.textInput, isDark && { color: colors.white }]} placeholder="Visitor's company" placeholderTextColor={colors.neutral[400]} value={visitorCompany} onChangeText={setVisitorCompany} />
              </View>
            </View>
            ...
            <View style={formStyles.fieldWrap}>
              <Text className="mb-2 font-inter text-xs font-bold text-primary-900 dark:text-primary-100 uppercase tracking-wider">
                Host Employee <Text className="text-danger-500">*</Text>
              </Text>
...
export function RecurringPassesScreen() {
  ...
  const items: RecurringPassItem[] = React.useMemo(() => {
    ...
    return raw.map((p: any) => ({
      id: p.id ?? '',
      passNumber: p.passNumber ?? p.code ?? '',
      visitorName: p.visitorName ?? p.visitor?.name ?? '',
      visitorCompany: p.visitorCompany ?? p.visitor?.company ?? '',
      passType: p.passType ?? p.type ?? '',
      validFrom: p.validFrom ?? p.startDate ?? '',
      validUntil: p.validUntil ?? p.endDate ?? '',
      status: p.status ?? 'ACTIVE',
      hostName: p.hostName ?? p.host?.name ?? '',
    }));
  }, [response]);
  ...
  const renderItem = ({ item, index }: { readonly item: RecurringPassItem; readonly index: number }) => (
    <PassCard item={item} index={index} onRevoke={() => handleRevoke(item)} fmt={fmt} />
  );
