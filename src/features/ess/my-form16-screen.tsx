    ...
    const renderItem = ({ item, index }: { item: any; index: number }) => (
        <Animated.View entering={FadeInDown.delay(index * 60).springify()} style={styles.card}>
            <View style={styles.cardHeader}>
                <View style={{ flex: 1 }}>
                    <Text className="font-inter text-sm font-bold text-primary-900">{item.financialYear ?? item.fy ?? `FY ${item.year ?? '--'}`}</Text>
                    {item.generatedAt && <Text className="font-inter text-xs text-neutral-500 mt-0.5">Generated: {item.generatedAt}</Text>}
                </View>
            ...
            <View style={styles.summaryRow}>
                <View style={styles.summaryItem}>
                    ...
                    <Text className="font-inter text-xs font-bold text-primary-950">{formatCurrency(item.grossSalary)}</Text>
                </View>
                <View style={styles.summaryItem}>
        ...
        <View style={{ flex: 1, backgroundColor: colors.white }}>
            <LinearGradient colors={[colors.gradient.start, colors.gradient.mid, colors.gradient.end]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.header, { paddingTop: insets.top + 8 }]}>
                <View style={styles.headerRow}>
                    ...
                    <Text className="font-inter text-lg font-bold text-white ml-3">My Form-16</Text>
                </View>
            </LinearGradient>
            <FlatList
                data={records}
