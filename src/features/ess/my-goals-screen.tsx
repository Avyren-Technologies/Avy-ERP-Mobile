    ...
    const renderItem = ({ item, index }: { item: any; index: number }) => (
        <Animated.View entering={FadeInDown.delay(index * 60).springify()} style={styles.card}>
            <View style={styles.cardHeader}>
                <View style={{ flex: 1 }}>
                    <Text className="font-inter text-sm font-bold text-primary-900 dark:text-primary-100" numberOfLines={2}>{item.title}</Text>
                    {item.cycle?.name && <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{item.cycle.name}</Text>}
                </View>
            ...
            {item.description && <Text className="font-inter text-xs text-neutral-600 dark:text-neutral-400 mt-2">{item.description}</Text>}
