...
function StarRow({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
    return (
        <View style={{ marginBottom: 14 }}>
            <Text className="mb-1 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">{label}</Text>
        ...
        <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            <View style={{ flex: 1, backgroundColor: 'rgba(8, 15, 40, 0.32)' }}>
                ...
                <View style={[styles.formSheet, { paddingBottom: insets.bottom + 20, marginTop: insets.top + 40 }]}>
                    ...
                    <Text className="font-inter text-lg font-bold text-primary-950 dark:text-white mb-4">Training Feedback</Text>
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" style={{ flex: 1 }}>
                        ...
                        <View style={{ marginBottom: 14 }}>
                            <Text className="mb-1.5 font-inter text-xs font-bold text-primary-900 dark:text-primary-100">Improvement Suggestions</Text>
                            <View style={[styles.inputWrap, { height: 80 }]}>
...
function MaterialsSection({ trainingId }: { trainingId: string }) {
    ...
    return (
        <View style={{ marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: colors.neutral[100] }}>
            <Text className="font-inter text-xs font-bold text-primary-900 dark:text-primary-100 mb-2">Materials</Text>
            {materials.map((mat: any) => {
                ...
                return (
                    <View key={mat.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 }}>
                        ...
                        <Text className="font-inter text-xs text-primary-950 dark:text-white flex-1" numberOfLines={1}>{mat.name ?? mat.title ?? 'Untitled'}</Text>
                        {(mat.url || mat.fileUrl) && (
                ...
                <Pressable onPress={() => toggleExpand(item)} style={[styles.card, isExpanded && { borderColor: colors.primary[200] }]}>
                    <View style={styles.cardHeader}>
                        <View style={{ flex: 1 }}>
                            ...
                            {item.trainerName && <Text className="font-inter text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Trainer: {item.trainerName}</Text>}
                        </View>
                    ...
                    {item.description && <Text className="font-inter text-xs text-neutral-600 dark:text-neutral-400 mt-2" numberOfLines={isExpanded ? undefined : 2}>{item.description}</Text>}
...
const createStyles = (isDark: boolean) => StyleSheet.create({
    ...
    feedbackBtn: { alignSelf: 'flex-start', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, backgroundColor: isDark ? colors.primary[900] : colors.primary[50], borderWidth: 1, borderColor: colors.primary[200] },
