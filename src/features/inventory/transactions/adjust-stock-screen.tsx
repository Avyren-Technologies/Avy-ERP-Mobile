...
export function AdjustStockScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useAdjustments({ page, limit: 25 });
    ...
    <Animated.View entering={FadeInDown.delay(index * 30).duration(300)}>
      <View style={styles.card}>
        ...
        <Text className="text-xs font-inter text-neutral-600 mt-1">
          {item.adjustmentType || item.type || 'ADJUSTMENT'} | {item.reasonCode?.name || item.reason || '--'}
        </Text>
        <View style={styles.cardRow}>
          <Text className="text-xs font-inter text-neutral-500">{item.createdAt ? fmt.dateTime(item.createdAt) : '--'}</Text>
          <Text className="text-sm font-bold font-inter text-neutral-900">{Number(item.quantity ?? item.qty ?? 0).toLocaleString()}</Text>
        </View>
      </View>
    </Animated.View>
  ), [fmt]);
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      <LinearGradient colors={[colors.gradient.start, colors.gradient.mid, colors.gradient.end]} style={{ paddingTop: insets.top + 8, paddingBottom: 16, paddingHorizontal: 16 }}>
        <View style={styles.headerRow}>
          ...
          <Text className="text-xl font-bold text-white font-inter ml-3">Stock Adjustments</Text>
        </View>
      </LinearGradient>
...
function CreateAdjustSheet({ onClose }: { onClose: () => void }) {
  const createMutation = useCreateAdjustStock();
  const { data: rcData } = useReasonCodes({ type: 'ADJUSTMENT' });
  const reasonCodes = (rcData as any)?.data || [];
  const [form, setForm] = useState({ partId: '', warehouseId: '', quantity: '', reasonCodeId: '', remarks: '' });

  const canSubmit = form.partId && form.warehouseId && Number(form.quantity) !== 0;

  const handleSubmit = () => {
    createMutation.mutate({
      partId: form.partId, warehouseId: form.warehouseId,
      quantity: Number(form.quantity), reasonCodeId: form.reasonCodeId || undefined,
      ...
      <View style={styles.sheet}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
          <View style={styles.sheetHeader}>
            <Text className="text-lg font-bold font-inter text-neutral-900">New Adjustment</Text>
            <TouchableOpacity onPress={onClose}><X color={colors.neutral[400]} size={20} /></TouchableOpacity>
          </View>
          {[{ key: 'partId', label: 'Part ID *' }, { key: 'warehouseId', label: 'Warehouse ID *' }, { key: 'quantity', label: 'Quantity (+/-) *', numeric: true }, { key: 'remarks', label: 'Remarks' }].map((f) => (
          ...
          {reasonCodes.length > 0 && (
            <View style={{ marginTop: 12 }}>
              <Text className="text-xs font-medium font-inter text-neutral-600 mb-1">Reason Code</Text>
              <View style={styles.pickerWrap}>
                {reasonCodes.map((rc: any) => (
                  <TouchableOpacity key={rc.id} onPress={() => setForm({ ...form, reasonCodeId: rc.id })} style={[styles.pickerItem, form.reasonCodeId === rc.id && styles.pickerItemActive]}>
                    <Text className={`text-xs font-inter ${form.reasonCodeId === rc.id ? 'text-white font-bold' : 'text-neutral-700'}`}>{rc.code}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || createMutation.isPending} style={[styles.submitBtn, (!canSubmit || createMutation.isPending) && { opacity: 0.5 }]}>
            {createMutation.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Text className="text-sm font-bold font-inter text-white">Create Adjustment</Text>}
          </TouchableOpacity>
        </ScrollView>
      </View>
    </View>
