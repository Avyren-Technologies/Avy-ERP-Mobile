...
export function CustomerReturnScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useCustomerReturns({ page, limit: 25 });
  ...
  const renderItem = useCallback(({ item, index }: { item: any; index: number }) => (
    <Animated.View entering={FadeInDown.delay(index * 30).duration(300)}>
      <View style={styles.card}>
        ...
        <Text className="text-xs font-inter text-neutral-600 mt-1">Customer: {item.customerName || '--'}</Text>
        ...
        {item.status === 'PENDING_INSPECTION' && (
          <TouchableOpacity
            onPress={() => inspectMutation.mutate({ id: item.id, data: { disposition: 'RETURN_TO_STOCK' } })}
            disabled={inspectMutation.isPending}
            style={[styles.actionBtn, inspectMutation.isPending && { opacity: 0.5 }]}
          >
            <Text className="text-xs font-bold font-inter text-primary-600">Inspect</Text>
          </TouchableOpacity>
        )}
      </View>
    </Animated.View>
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      <LinearGradient colors={[colors.gradient.start, colors.gradient.mid, colors.gradient.end]} style={{ paddingTop: insets.top + 8, paddingBottom: 16, paddingHorizontal: 16 }}>
        <View style={styles.headerRow}>
          ...
          <Text className="text-xl font-bold text-white font-inter ml-3">Customer Returns</Text>
        </View>
      </LinearGradient>
...
function CreateReturnSheet({ onClose }: { onClose: () => void }) {
  const createMutation = useCreateCustomerReturn();
  const [form, setForm] = useState({ warehouseId: '', customerName: '', partId: '', quantity: '', reason: '' });
  ...
  const handleSubmit = () => {
    createMutation.mutate({
      ...
      lineItems: [{ partId: form.partId, quantity: Number(form.quantity), returnReason: form.reason || undefined }],
    }, { onSuccess: onClose });
  };
      ...
      <View style={styles.sheet}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
          <View style={styles.sheetHeader}>
            <Text className="text-lg font-bold font-inter text-neutral-900">New Customer Return</Text>
            <TouchableOpacity onPress={onClose}><X color={colors.neutral[400]} size={20} /></TouchableOpacity>
          </View>
          {[{ key: 'warehouseId', label: 'Warehouse ID *' }, { key: 'customerName', label: 'Customer Name' }, { key: 'partId', label: 'Part ID *' }, { key: 'quantity', label: 'Quantity *', numeric: true }, { key: 'reason', label: 'Return Reason' }].map((f) => (
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || createMutation.isPending} style={[styles.submitBtn, (!canSubmit || createMutation.isPending) && { opacity: 0.5 }]}>
            {createMutation.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Text className="text-sm font-bold font-inter text-white">Log Return</Text>}
          </TouchableOpacity>
        </ScrollView>
      </View>
    </View>
