...
export function PickItemsScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = usePickItems({ page, limit: 25 });
  ...
  const renderItem = useCallback(({ item, index }: { item: any; index: number }) => (
    <Animated.View entering={FadeInDown.delay(index * 30).duration(300)}>
      <View style={styles.card}>
        ...
        <Text className="text-xs font-inter text-neutral-600 mt-1">{item.partNumber || item.part?.partNumber || '--'} - Qty: {Number(item.quantity ?? item.qty ?? 0).toLocaleString()}</Text>
        <Text className="text-xs font-inter text-neutral-500">{item.createdAt ? fmt.dateTime(item.createdAt) : '--'}</Text>
        {item.status === 'CREATED' && (
          <TouchableOpacity
            onPress={() => confirmMutation.mutate({ id: item.id, data: {} })}
            disabled={confirmMutation.isPending}
            style={[styles.actionBtn, confirmMutation.isPending && { opacity: 0.5 }]}
          >
            <Text className="text-xs font-bold font-inter text-primary-600">Confirm Pick</Text>
          </TouchableOpacity>
        )}
      </View>
    </Animated.View>
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      <LinearGradient colors={[colors.gradient.start, colors.gradient.mid, colors.gradient.end]} style={{ paddingTop: insets.top + 8, paddingBottom: 16, paddingHorizontal: 16 }}>
        <View style={styles.headerRow}>
          ...
          <Text className="text-xl font-bold text-white font-inter ml-3">Pick Items</Text>
        </View>
      </LinearGradient>
...
function CreatePickSheet({ onClose }: { onClose: () => void }) {
  const createMutation = useCreatePickItems();
  const [form, setForm] = useState({ partId: '', warehouseId: '', quantity: '', remarks: '' });
  const canSubmit = form.partId && form.warehouseId && Number(form.quantity) > 0;

  const handleSubmit = () => {
    createMutation.mutate({
      partId: form.partId, warehouseId: form.warehouseId,
      quantity: Number(form.quantity), remarks: form.remarks || undefined,
    }, { onSuccess: onClose });
  };
      ...
      <View style={styles.sheet}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
          <View style={styles.sheetHeader}>
            <Text className="text-lg font-bold font-inter text-neutral-900">New Pick List</Text>
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || createMutation.isPending} style={[styles.submitBtn, (!canSubmit || createMutation.isPending) && { opacity: 0.5 }]}>
            {createMutation.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Text className="text-sm font-bold font-inter text-white">Create Pick List</Text>}
          </TouchableOpacity>
        </ScrollView>
      </View>
    </View>
