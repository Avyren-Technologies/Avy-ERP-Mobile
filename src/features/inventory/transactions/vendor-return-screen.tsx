...
export function VendorReturnScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useVendorReturns({ page, limit: 25 });
  ...
  const renderItem = useCallback(({ item, index }: { item: any; index: number }) => (
    <Animated.View entering={FadeInDown.delay(index * 30).duration(300)}>
      <View style={styles.card}>
        ...
        <Text className="text-xs font-inter text-neutral-600 mt-1">Vendor: {item.vendorName || item.supplierName || '--'}</Text>
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      <LinearGradient colors={[colors.gradient.start, colors.gradient.mid, colors.gradient.end]} style={{ paddingTop: insets.top + 8, paddingBottom: 16, paddingHorizontal: 16 }}>
        <View style={styles.headerRow}>
          ...
          <Text className="text-xl font-bold text-white font-inter ml-3">Vendor Returns</Text>
        </View>
      </LinearGradient>
...
function CreateVendorReturnSheet({ onClose }: { onClose: () => void }) {
  const createMutation = useCreateVendorReturn();
  const [form, setForm] = useState({ warehouseId: '', vendorName: '', partId: '', quantity: '', reason: '' });
  const canSubmit = form.warehouseId && form.partId && Number(form.quantity) > 0;

  const handleSubmit = () => {
    createMutation.mutate({
      warehouseId: form.warehouseId, vendorName: form.vendorName || undefined,
      ...
      <View style={styles.sheet}>
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
          <View style={styles.sheetHeader}>
            <Text className="text-lg font-bold font-inter text-neutral-900">New Vendor Return</Text>
            <TouchableOpacity onPress={onClose}><X color={colors.neutral[400]} size={20} /></TouchableOpacity>
          </View>
          {[{ key: 'warehouseId', label: 'Warehouse ID *' }, { key: 'vendorName', label: 'Vendor Name' }, { key: 'partId', label: 'Part ID *' }, { key: 'quantity', label: 'Quantity *', numeric: true }, { key: 'reason', label: 'Return Reason' }].map((f) => (
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={!canSubmit || createMutation.isPending} style={[styles.submitBtn, (!canSubmit || createMutation.isPending) && { opacity: 0.5 }]}>
            {createMutation.isPending ? <ActivityIndicator color="#fff" size="small" /> : <Text className="text-sm font-bold font-inter text-white">Create Return</Text>}
          </TouchableOpacity>
        </ScrollView>
      </View>
    </View>
