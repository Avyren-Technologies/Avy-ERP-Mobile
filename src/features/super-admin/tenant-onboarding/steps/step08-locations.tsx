...
function FacilityTypeSelector({
    ...
    const isCustomSelected =
        selected === 'Custom...' ||
        (!FACILITY_TYPES.slice(0, -1).includes(selected) && selected !== '');
    ...
    const displaySelected =
        isCustomSelected && selected !== 'Custom...' ? selected : selected;
...
function PlantBranchCard({
    ...
    return (
        <Animated.View entering={FadeIn.duration(250)} style={[S.itemCard, isHQ && S.hqCard]}>
            {/* Card Header */}
...
export function Step8Locations({
    locations,
    setLocations,
    errors,
}: {
